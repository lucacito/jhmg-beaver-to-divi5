...
export const BASE = process.env.PLAYWRIGHT_BASE_URL || 'http://localhost:8010';
...
/** Creates a page carrying a fixture layout (path relative to fixtures/, no .json). */
export function createBeaverPage(fixture: string, title: string): string {
  const id = wp(`wp post create --post_type=page --post_status=publish --post_title=${shellEscape(title)} --porcelain --allow-root`).trim();
  wp(`FIXTURE=${shellEscape(fixture)} PAGE_ID=${shellEscape(id)} wp eval-file /tmp/set-beaver-data.php --allow-root`);
  return id;
}
