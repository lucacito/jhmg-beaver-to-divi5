...
|---|---|---|
| `jhmg-converter-for-beaver-builder-to-divi` (free) | WordPress.org SVN | plugin header + `BDC_PLUGIN_VERSION` + readme `Stable tag` |
...
3. `npm run i18n` when user-facing strings changed.
4. Free: `rsync -a --delete --exclude='.DS_Store' --exclude='.svn' plugin/jhmg-converter-for-beaver-builder-to-divi/ wporg-svn/trunk/`,
...
5. Pro: zip `plugin/jhmg-converter-for-beaver-builder-to-divi-pro/` (no `.DS_Store`), upload to divi5lab.com,
...
6. `AdminPage::PRO_PRICE` must match the divi5lab.com listing; nothing enforces it.
