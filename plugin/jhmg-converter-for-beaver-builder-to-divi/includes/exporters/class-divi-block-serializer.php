...
/**
 * Serializes the converted block tree to Divi 5's storage format: WordPress
...
class DiviBlockSerializer {
    ...
    /** A section holds rows; anything else is wrapped in a row (and column) on the way out. */
    ...
    private function encodeAttrs( array $attrs ): string {
        // Block comment delimiters cannot contain "--"; JSON-escaping "<" and ">"
        // (which wp_json_encode does not do by default) keeps HTML content safe
        // inside the comment, the same way Divi itself stores it.
        $json = wp_json_encode( $attrs, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_HEX_TAG );
        return $json === false ? '{}' : $json;
    }

    /**
     * Divi's FlexboxMigration (hooked on the_content) rewrites blocks that carry
     * no builderVersion; stamping the running version keeps it off ours.
     */
