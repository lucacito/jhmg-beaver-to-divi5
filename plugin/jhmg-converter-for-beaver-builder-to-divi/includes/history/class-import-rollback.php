...
class ImportRollback {
    ...
    public function rollback( string $import_id ): array {
        ...
        if ( ! self::trash_available() ) {
            return [ 'trashed' => 0, 'skipped' => count( $run['post_ids'] ?? [] ), 'trash_unavailable' => true ];
        }
    ...
    public function notice_markup( array $result ): string {
        if ( ! empty( $result['trash_unavailable'] ) ) {
            return '<div class="notice notice-warning is-dismissible bdc-rollback-notice"><p>'
                . esc_html__( 'Undo did not run: this site is configured to empty the Trash immediately, so nothing could be trashed. No pages were changed.', 'jhmg-converter-for-beaver-builder-to-divi' )
                . '</p></div>';
        }
        ...
        if ( $trashed > 0 ) {
            $parts[] = esc_html( sprintf(
                ...
                _n( '%d page was moved to the Trash. You can restore it from Pages → Trash if needed.', '%d pages were moved to the Trash. You can restore them from Pages → Trash if needed.', $trashed, 'jhmg-converter-for-beaver-builder-to-divi' ),
                $trashed
            ) );
        }
        if ( $skipped > 0 ) {
            $parts[] = esc_html( sprintf(
                ...
                _n( '%d page was skipped because it is no longer owned by this plugin (already gone, or replaced since the conversion).', '%d pages were skipped because they are no longer owned by this plugin (already gone, or replaced since the conversion).', $skipped, 'jhmg-converter-for-beaver-builder-to-divi' ),
                $skipped
            ) );
        }
        if ( empty( $parts ) ) {
            $parts[] = esc_html__( 'Nothing to undo: this run had already been undone or had no pages left to trash.', 'jhmg-converter-for-beaver-builder-to-divi' );
        }

        return '<div class="notice notice-success is-dismissible bdc-rollback-notice"><p>' . implode( ' ', $parts ) . '</p></div>';
    }
