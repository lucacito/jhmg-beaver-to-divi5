<?php
/**
 * Plugin Name: JHMG Converter For Beaver Builder to Divi 5
 * Plugin URI:  https://divi5lab.com/plugins/beaver-builder-to-divi-5
 * Description: Converts Beaver Builder page layouts into native Divi 5 block content.
 * Version:     1.0.0
 * Author:      Lucas Lopvet
 * Author URI:  https://jhmediagroup.com
 * License:     GPLv2 or later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: jhmg-converter-for-beaver-builder-to-divi
 * Domain Path: /languages
 * Requires at least: 5.9
 * Requires PHP: 8.0
 */

if ( ! defined( 'ABSPATH' ) ) {
    exit;
}

define( 'BBDC_PLUGIN_FILE', __FILE__ );
define( 'BBDC_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );
define( 'BBDC_PLUGIN_URL', plugin_dir_url( __FILE__ ) );

defined( 'BBDC_PLUGIN_VERSION' ) || define( 'BBDC_PLUGIN_VERSION', '1.0.0' );

require_once BBDC_PLUGIN_DIR . 'includes/helpers/class-autoloader.php';

// Activation runs inside an admin request, so the theme is loaded and Divi's
// version is readable — unlike at plugins_loaded, where it is not yet.
register_activation_hook( __FILE__, [ \BeaverDivi5Converter\Helpers\DiviRequirement::class, 'on_activation' ] );

\BeaverDivi5Converter\Plugin::instance()->init();
